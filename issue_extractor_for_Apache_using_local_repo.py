...
os.chdir('E:\github\struts')
command = 'git log --numstat --diff-filter="ACDMRT" -1 -U '
    ...
    try:
        ...
        if hasCommit:
            ...
            for commit in commits:
                output = str(subprocess.check_output(command + commit['id']))
                ...
                for line in output_split:
                    if len(line)>0 and line[0].isdigit() and  line.endswith(".java"):
                        patches.append({'filename': line.split("\\t")[2], 'insertion': line.split("\\t")[0], 'deletion': line.split("\\t")[1]})
